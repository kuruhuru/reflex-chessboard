...
_default_excludes = [_app_root / ".web", _app_root / ".react-router", _app_root / "dist", _app_root / "build"]
_exclude_env = os.environ.get("REFLEX_HOT_RELOAD_EXCLUDE_PATHS", "")
if not _exclude_env:
    os.environ["REFLEX_HOT_RELOAD_EXCLUDE_PATHS"] = ":".join(str(p) for p in _default_excludes if p.exists())

# Include the monorepo component source for hot-reload while developing the demo.
# Otherwise, changes under `../custom_components/` won't trigger a recompile.
_repo_root = _app_root.parent
_default_includes = [_repo_root / "custom_components"]
_include_env = os.environ.get("REFLEX_HOT_RELOAD_INCLUDE_PATHS", "")
if not _include_env:
    os.environ["REFLEX_HOT_RELOAD_INCLUDE_PATHS"] = ":".join(str(p) for p in _default_includes if p.exists())
